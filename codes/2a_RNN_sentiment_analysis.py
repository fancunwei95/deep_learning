...
for epoch in range(no_of_epochs):
    ...
    # ## test
        ...
        for i in range(0, L_Y_test, batch_size):
            if i+batch_size >= len(x_test):
                continue
            x_input2 = [x_test[j] for j in I_permutation[i:min(len(x_test),i+batch_size)]]
            sequence_length_test = 200
            x_input = np.zeros((batch_size,sequence_length_test),dtype=np.int)
            this_batch_size = len(x_input2)
            for j in range(this_batch_size):
                x = np.asarray(x_input2[j])
                sl = x.shape[0]
                if(sl < sequence_length_test):
                    x_input[j,0:sl] = x
                else:
                    start_index = np.random.randint(sl-sequence_length_test+1)
                    x_input[j,:] = x[start_index:(start_index+sequence_length_test)]
...
torch.save(model,'rnn_'+str(ID)+'.model')
data = [train_loss,train_accu,test_accu]
data = np.asarray(data)
np.save('data_'+str(ID)+'.npy',data)
